...
export function can(permission: string[]){
    return async (request: Request, response: Response, next: NextFunction) => {
        try{
            ...
            if(!userId) throw new Error("not authorization")

            let getAllPermission = await Prisma.permissionRoles.findMany({
                where: {
                    userId
                },
                include: {
            ...
            if(getAllPermission.length <= 0) throw new Error("not authorization");
            ...
            if(!authorized) throw new Error("not authorization");
